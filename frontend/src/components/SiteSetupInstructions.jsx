...
export default function SiteSetupInstructions({ site, onClose }) {
  ...
  --name logradar-docker-logs-collector \\
  ...
  ghcr.io/gsotti/logradar-docker-logs-collector:latest`;

    const statsCollectorCmd = `docker run -d \\
  --name logradar-docker-stats-collector \\
  --restart unless-stopped \\
  -v /var/run/docker.sock:/var/run/docker.sock:ro \\
  -e LOGRADAR_API_URL="${window.location.origin}" \\
  -e API_TOKEN="${site.api_token}" \\
  -e COLLECTION_INTERVAL_MS="60000" \\
  ghcr.io/gsotti/logradar-docker-stats-collector:latest`;

    const composeFile = `version: '3.8'

services:
  logradar-docker-logs-collector:
    image: ghcr.io/gsotti/logradar-docker-logs-collector:latest
    container_name: logradar-docker-logs-collector
    restart: unless-stopped
    environment:
  ...
  logradar-docker-stats-collector:
    image: ghcr.io/gsotti/logradar-docker-stats-collector:latest
    container_name: logradar-docker-stats-collector
    restart: unless-stopped
    environment:
      LOGRADAR_API_URL: "${window.location.origin}"
      API_TOKEN: "${site.api_token}"
