...
export default function UptimeMonitorsView({ siteId }) {
  ...
  const fetchData = async () => {
    try {
      const monitorsData = await api.get(`/uptime/monitors?site_id=${siteId}`);
      setMonitors(monitorsData || []);

      // Fetch recent checks for each monitor
      const checksPromises = (monitorsData || []).map(async (m) => {
        try {
          const history = await api.get(`/uptime/monitors/${m.id}/history?limit=30`);
  ...
  const handleCreate = async (form) => {
    setSaving(true);
    try {
      await api.post('/uptime/monitors', { ...form, site_id: parseInt(siteId) });
  ...
  const handleManualCheck = async (monitor) => {
    setCheckingId(monitor.id);
    try {
      const result = await api.post(`/uptime/monitors/${monitor.id}/check`);
