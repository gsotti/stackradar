...
export default function SystemFilter({ className = '' }) {
  const {
    selectedTenant,
    selectedSite,
    selectedEnvironment,
  ...
  useEffect(() => {
    const fetchSystems = async () => {
      ...
      try {
        ...
        const systems = await api.get(`/systems?${params}`);
  ...
  return (
    <select
      ...
      disabled={loading}
      className={`px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-all ${className}`}
    >
